...
async function connectToWhatsApp() {
		...
		if (type === "notify") {
			...
			if (!msg.key.fromMe) {
				...
				if (!msg.key.fromMe && pesanMasuk === "ping") {
					await sock.sendMessage(noWa, { text: noWa + " Pong" }, { quoted: msg });
				}
			}
		}
	});
	...
	app.post('/send-code', async (req, res) => {
		...
		if (!phoneNumber) {
			return res.status(400).json({ error: 'Nomor WhatsApp yang valid diperlukan!' });
		}
		...
		verificationCodes[index] = { code, expiresAt: Date.now() + CODE_EXPIRATION_TIME };
		...
		try {
			...
			res.json({ success: true, index, message: 'Kode verifikasi dikirim ke WhatsApp!' });
		} catch (error) {
	...
	app.post("/verify-code", (req, res) => {
		...
		if (!verificationCodes[index]) {
			return res.json({ success: false, message: "Kode tidak ditemukan atau sudah kadaluwarsa!" });
		}
		...
		if (Date.now() > storedData.expiresAt) {
			...
			return res.json({ success: false, message: "Kode sudah kadaluwarsa!" });
		}

		// Cek apakah kode cocok
		if (storedData.code === code) {
			res.json({ success: true, message: "Verifikasi berhasil! Pendaftaran sukses." });
		} else {
	...
	app.post('/send-grub', async (req, res) => {
		...
		idgrubnya = ["120363390721446244@g.us", "120363408604028416@g.us", "120363392500574290@g.us", "120363394223609294@g.us", "120363410208252797@g.us"]
		try {
			...
			res.json({ success: true, message: 'Pemberitahuan dikirim ke semua grup WhatsApp!' });
		} catch (error) {
